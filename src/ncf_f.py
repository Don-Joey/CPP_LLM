...
class EnhancedNeuralCollaborativeFiltering(nn.Module):
    ...
    def forward(self, model_ids, task_ids, task_features, model_features):
        model_embed = self.model_embedding(model_ids)
        task_embed = self.task_embedding(task_ids)
        gmf = model_embed * task_embed
        ...
        model_cate_feat1 = self.model_feat1_embedding(model_features[:,-4].int())
        model_cate_feat2 = self.model_feat2_embedding(model_features[:,-3].int())
        model_cate_feat3 = self.model_feat3_embedding(model_features[:,-2].int())
        model_cate_feat4 = self.model_feat4_embedding(model_features[:,-1].int())
        model_features = model_numerical_feat+model_cate_feat1+model_cate_feat2+model_cate_feat3+model_cate_feat4

        task_feat1 = self.task_feat1_embedding(task_features[:, 0])
        task_feat2 = self.task_feat2_embedding(task_features[:, 1])
        task_feat3 = self.task_feat3_embedding(task_features[:, 2])
        task_feat4 = self.task_feat4_embedding(task_features[:, 3])
        task_features = task_feat1+task_feat2+task_feat3+task_feat4
        mlp = torch.cat((model_mlp+model_features, task_mlp+task_features), -1)
