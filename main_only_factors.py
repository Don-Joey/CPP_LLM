...
from src.ncf_f_noid import EnhancedNeuralCollaborativeFiltering
...
def train(model, optimizer, criterion, data_loader, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        for model, task, rating, model_feature, bench_feature in data_loader:
            ...
            optimizer.zero_grad()
            prediction = model(model_feature, bench_feature)
...
def main(data_name, mask_size, model_name, random_state):
    model_save_root = "/data/qiyuan/bench_test/models/"
    csv_path = "./data/crowdsource_performance.csv"
    ...
    for random_state in random_states:
        ...
        model_ids, task_ids, ratings, train_scales, train_min_values, train_bench_features, train_model_features = load_data(score_data, train_array_positions, scale, min_value)
        valid_model_ids, valid_task_ids, valid_ratings, valid_scales, valid_min_values, valid_bench_features, valid_model_features = load_data(score_data, validate_array_positions, scale, min_value)
        # Hyperparameters
        learning_rate = 0.01
        num_epochs = 250000
        ...
        elif model_name == "enhanced_neural_collaborative_filtering":
            torch.save(model.state_dict(), model_save_root+"noid/"+data_name+"-"+model_name+"-"+str(num_epochs)+"-"+str(random_state)+"-"+str(learning_rate)[2:]+"-"+str(num_factors)+"-SGD-mask"+str(int(mask_size*100))+'.pth')
